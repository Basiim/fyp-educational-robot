...
void loop(){
  ...
  {
    String save;
    int sizeOfloop=(indexEndloop-indexloop)-1; 
    String TempArray[sizeOfloop]; 
    int noOfiterations=0; //number of iterations in a loop 
    int s=0; //variable for index of temporary array 
    noOfiterations=loopString.toInt();
    Serial.println(noOfiterations);
    int saveIndexmainString = 0; //integer for saving the last used index of NewMainString
    int indexOfnewString=0; //index of the new main string 
    int d=0; //number of commands other than loop that are already saved in newMainstring
    String command; //the commands of loop in queue
    Serial.println("I'm in command handler");
    for(int i=0; i<=x; i++)
    { 
      Serial.println("I'm in command handler checkpost2");
      Serial.println("i'm first loop");
      indexOfnewString= d+(noOfiterations*sizeOfloop); // indexOfnewString 
      command= Dequeue(&front1,&rear1);
      if(command == "loop") 
      {
        command= Dequeue(&front1,&rear1);
          i++;
        while(command!="endloop") 
        {
          Serial.println("I'm in command handler checkpost3");
          Serial.println("i'm inner loop");
         TempArray[s]=command; // forward is saved in temparray[0]
         s+=1; // s = 1
         command= Dequeue(&front1,&rear1);
          i++;
        }
        ...
        {
          for(int b=0; b<sizeOfloop; b++){
             Enqueue ( &front2,&rear2,TempArray[b]); // forward is saved in main[0]  
          }
        }
      }
      else{
        Enqueue (&front2,&rear2,command);
        saveIndexmainString =indexOfnewString+1;
        d++;
         Serial.println("hi, I am displayQueue for 2nd linked list");
         displayQueue(&front2,&rear2);
        
      }
     /// Enqueue (&front2,&rear2,"final");
    }  
   /** for(int i=0; i<indexOfnewString; i++)
    {
      save= Dequeue(&front2,&rear2);
      if (save == "forward") {
                   routine_calls(90);
            ...
            }else if (save == "backward") {
               routine_calls(180);
               Serial.println("i'm not here");
            } 
            else if (save == "left") {
